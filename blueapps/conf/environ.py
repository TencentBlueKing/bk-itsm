...
# 由用户初始化
from config import APP_CODE, BASE_DIR, RUN_VER, SECRET_KEY, celery_app  # noqa

try:
    from config import BK_URL, BK_PAAS_HOST
except ImportError:
    BK_URL = None
...
try:
    ...
    raise ImportError('unknown RUN_VER: %s' % RUN_VER)
...
# 分享链接PC端
FRONTEND_URL = os.environ.get("BKAPP_FRONTEND_URL") or os.path.join(BK_PAAS_HOST,
                                                                    os.environ.get("BKAPP_ITSM_URL",
                                                                                   "o/bk_itsm/"))
...
# 平台微信 URL 域名
WEIXIN_BK_URL = os.getenv('BKPAAS_WEIXIN_URL', FRONTEND_URL)
