...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    @property
    def is_wiki_superuser(self):
        # 拿出来是个字符串，为了能直接作为if的判断条件，所以要int一下
        is_wiki_superuser = self.get_property('is_wiki_superuser')
        if is_wiki_superuser is not None:
            return int(is_wiki_superuser)
        ...
        if v_info_cnt == 0:
            ...
            if cur.updated_at >= now - datetime.timedelta(
                ...
                if cur.updated_at < now - datetime.timedelta(
                    ...
                    if ret['result']:
                        ret['message'] = '已生成的验证码，重发成功'
                else:
                    # 等待时间不足，不重发
                    ret = {'result': False, 'message': '暂不能重发验证码，请稍等'}
            else:
                # 已过期，重新生成并重发
                ...
                if ret['result']:
                    ret['message'] = '重新生成验证码，发送成功'
        else:
            logger.error(
                'found more than one code of the user->[%s]' % self.id
            )
            ret = {'result': False, 'message': '数据库中的验证码异常'}
        return ret
