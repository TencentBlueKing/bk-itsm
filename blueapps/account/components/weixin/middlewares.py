...
class WeixinLoginRequiredMiddleware(MiddlewareMixin):
    def process_view(self, request, view, args, kwargs):
        ...
        if not (login_exempt or request.user.is_authenticated):
            ...
            if form.is_valid():
                ...
                logger.debug(
                    "微信请求链接，检测到微信验证码，code：{}，state：{}".format(
                        code,
                        state
                    )
                )
                ...
                logger.debug(
                    "微信请求链接，未检测到微信验证码，url：{}，params：{}".format(
    ...
    def valid_state(self, request, state, expires_in=60):
        ...
        if not raw_state or raw_state != state:
            logger.warning(
                ("验证 WEIXIN 服务器返回信息，state 不一致，"
                 "WEIXIN_OAUTH_STATE=%s，state=%s") % (
                    raw_state, state
                )
            )
            return False
        ...
        if not raw_timestamp or time.time() - raw_timestamp > expires_in:
            logger.warning(
                ("验证 WEIXIN 服务器返回信息，state 过期，"
                 "WEIXIN_OAUTH_STATE_TIMESTAMP=%s") % (
                    raw_timestamp
                )
            )
            return False
