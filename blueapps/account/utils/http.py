...
def send(url, method, params, timeout=None, **kwargs):
    ...
    try:
        if method.upper() == 'GET':
            ...
            raise Exception("异常请求方式，%s" % method)
    except requests.exceptions.Timeout:
        err_msg = ("请求超时，url={}，method={}，params={}，timeout={}".format(
            url, method, params, timeout))
        raise ApiNetworkError(err_msg)

    logger.debug('请求记录, url={}, method={}, params={}, response={}'.format(
        url, method, params, response))

    if response.status_code != requests.codes.ok:
        err_msg = ("返回异常状态码，status_code=%s，url=%s，method=%s，"
                   "params=%s" % (response.status_code, url, method,
                                  json.dumps(params)))
    ...
    try:
        ...
        err_msg = ("返回内容不符合 JSON 格式，url=%s，method=%s，params=%s，error=%s，"
                   "response=%s" % (url, method, json.dumps(params),
