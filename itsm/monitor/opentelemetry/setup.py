...
from itsm.monitor.opentelemetry.instrumentor import BKAppInstrumentor
    ...
    if settings.ENVIRONMENT == "dev":
        ...
        trace.set_tracer_provider(
            TracerProvider(
                resource=Resource.create(
                    {
                        SERVICE_NAME: os.getenv("BKAPP_OTEL_SERVICE_NAME")
        ...
        trace.set_tracer_provider(
            tracer_provider=TracerProvider(
                resource=Resource.create(
                    {
                        "service.name": os.getenv("BKAPP_OTEL_SERVICE_NAME")
                        or settings.APP_CODE,
                        "bk_data_id": int(os.getenv("BKAPP_OTEL_BK_DATA_ID")),
                    },
                ),
                sampler=_KNOWN_SAMPLERS[
                    os.getenv("BKAPP_OTEL_SAMPLER", "parentbased_always_off")
                ],
            )
        )
