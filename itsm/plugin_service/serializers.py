...
class PluginDetailQuerySerializer(PluginCodeQuerySerializer):
    ...
    with_app_detail = serializers.BooleanField(
        help_text="是否返回插件APP信息", required=False, default=False
    )
...
class PluginListQuerySerializer(serializers.Serializer):
    ...
    limit = serializers.IntegerField(
        help_text="分页配置，接口一次最多100条", required=False, default=100
    )
...
class MetaResponseSerializer(StandardResponseSerializer):
    class MetaSerializer(serializers.Serializer):
        ...
        versions = serializers.ListField(
            help_text="版本列表", child=serializers.CharField(help_text="版本号")
        )
    ...
    class DetailDataSerializer(serializers.Serializer):
        class SchemaSerializer(serializers.Serializer):
            ...
            required = serializers.ListField(
                help_text="必填字段", child=serializers.CharField(help_text="字段 key")
            )
        ...
        app = PluginAppDetailResponseSerializer(
            help_text="插件服务APP相关详情信息", required=False
        )
