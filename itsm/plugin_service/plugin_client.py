...
class PluginServiceApiClient:
    def __init__(self, plugin_code, plugin_host=None):
        ...
        self.plugin_host = plugin_host or os.path.join(
            result["data"]["url"], "bk_plugin/"
        )
    ...
    def dispatch_plugin_api_request(self, request_params, inject_headers=None):
        url, headers = self._prepare_apigw_api_request(
            path_params=["plugin_api_dispatch"]
        )
        if inject_headers:
        ...
        if any(
            [
                isinstance(data, InMemoryUploadedFile)
                for data in request_params["data"].values()
            ]
        ):
    ...
    def get_plugin_logs(plugin_code, trace_id, scroll_id=None):
        result = PluginServiceApiClient.get_paas_logs(
            plugin_code, trace_id, scroll_id, environment="prod"
        )
    ...
    def get_plugin_detail_list(search_term=None, limit=100, offset=0, **kwargs):
        ...
        return {
            "result": True,
            "message": None,
            "data": {"count": result["count"], "plugins": result["results"]},
        }
    ...
    def get_plugin_app_detail(plugin_code):
        ...
        if not info["deployed"]:
            return {
                "result": False,
                "data": None,
                "message": f"Plugin Service {plugin_code} does not deployed.",
            }
    ...
    def batch_get_paas_plugin_detailed_info(
        environment=None, limit=100, offset=0, search_term=None, **kwargs
    ):
        ...
        params.update(
            {"limit": limit, "offset": offset, "has_deployed": True, **kwargs}
        )
        if search_term:
            params.update({"search_term": search_term})
        return PluginServiceApiClient._request_api_and_error_retry(
    ...
    def get_paas_logs(plugin_code, trace_id, scroll_id=None, environment=None):
        """通过PaaS平台查询插件服务日志"""
        url, params = PluginServiceApiClient._prepare_paas_api_request(
            path_params=["system/bk_plugins", plugin_code, "logs"],
            environment=environment,
        )
